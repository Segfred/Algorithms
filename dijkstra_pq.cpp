...
//https://www.cnblogs.com/jiangshaoyin/p/9954937.html 证明过程看这里
//上面的链接里面提到了证明
//红框里面关键的一步“题意”是指dijstra的策略，
//每次都是从已经全局最短到未知的里面的一个顶点里的局部最短当作新的全局最短，所以disv <= disy不可能更大，（因为disv是选择的局部最短）
//狄杰斯特拉的特点在于每次的最小值必定都是从已知的最短路径连接过去的，可能是之前找到的最短路径一直没排上号，也可能是上一次才刚刚发现的最短路径
//就是贪心，局部最优当成全局最优，
//另一方面就是可以重复访问的问题。bfs只能访问一次，但是这个带权重，所以首次访问的可能不是最优的，需要重复访问。
